...
def load_muni_universe():
    ...
    with st.status("DATA ENGINE // INITIALIZING", expanded=True) as loader_status:
        ...
        try:
            ...
            console.add(f"MERGE // deduplicated holdings into {len(df):,} unique CUSIPs")
            console.add(f"READY // {loaded_now} ETF sources loaded in {elapsed:.1f}s")
            progress_bar.progress(100, text=f"READY • {len(df):,} unique municipal CUSIPs")
            ...
            loader_status.update(label="DATA ENGINE // ERROR", state="error", expanded=True)
            raise
    ...
    if load_clicked:
        with st.status("TREASURY ENGINE // CONNECTING", expanded=True) as status:
            ...
            try:
                ...
                status.update(label="TREASURY ENGINE // ERROR", state="error", expanded=True)
...
def render_treasury_source(meta):
    if meta.get("fallback"):
        st.warning(
            "WSJ individual Treasury rows were unavailable, so the official U.S. Treasury daily par yield curve is being used as a fallback."
        )
    st.caption(
...
def render_muni_screener(df, source_rows, etf_status, as_of):
    ...
    if failed_etfs:
        st.warning(f"{failed_etfs} ETF source(s) failed to load. Open Source Status below.")
    ...
    with row1[0]:
        cusip = st.text_input(
            "CUSIP", placeholder="e.g. 107431KW7", key="screen_cusip"
        ).strip().upper()
    available_states = sorted(
        x for x in df["State"].dropna().astype(str).unique() if x and x != "Unknown"
    )
    with row1[1]:
        states = st.multiselect(
            "States", available_states, placeholder="All states", key="screen_states"
        )
    with row1[2]:
        purchase_face = st.number_input(
            "Purchase face ($)", min_value=1000, max_value=10_000_000,
            value=5000, step=1000, key="screen_face"
        )
    with row1[3]:
        price_min = st.number_input(
            "Price min", min_value=0.0, max_value=200.0,
            value=None, step=0.01, placeholder="No minimum", key="screen_price_min"
        )
    with row1[4]:
        price_max = st.number_input(
            "Price max", min_value=0.0, max_value=200.0,
            value=None, step=0.01, placeholder="No maximum", key="screen_price_max"
        )

    row2 = st.columns(5)
    with row2[0]:
        coupon_min = st.number_input(
            "Coupon min (%)", min_value=0.0, max_value=20.0,
            value=None, step=0.01, key="screen_coupon_min"
        )
    with row2[1]:
        coupon_max = st.number_input(
            "Coupon max (%)", min_value=0.0, max_value=20.0,
            value=None, step=0.01, key="screen_coupon_max"
        )
    with row2[2]:
        ytw_min = st.number_input(
            "YTW min (%)", min_value=-10.0, max_value=50.0,
            value=None, step=0.01, key="screen_ytw_min"
        )
    with row2[3]:
        ytw_max = st.number_input(
            "YTW max (%)", min_value=-10.0, max_value=50.0,
            value=None, step=0.01, key="screen_ytw_max"
        )
    with row2[4]:
        sort_by = st.selectbox(
            "Sort",
            [
                "YTW: High → Low", "Price: Low → High", "Coupon: High → Low",
                "Maturity: Soonest", "ETF Coverage: High → Low",
            ],
            key="screen_sort",
        )
    ...
    with row3[0]:
        ...
        maturity_from = st.date_input("Maturity from", key="screen_mfrom") if use_mfrom else None
    with row3[1]:
        use_mto = st.checkbox("Maximum maturity", key="screen_use_mto")
        maturity_to = st.date_input("Maturity to", key="screen_mto") if use_mto else None
    with row3[2]:
    ...
    with row4[0]:
        nist_only = st.checkbox("No State Individual Income Tax", key="screen_nist")
    with row4[1]:
        new_issue = st.checkbox("New issues", key="screen_new")
    with row4[2]:
        new_days = st.selectbox(
            "New issue window", [30, 60, 90, 180], index=1,
            disabled=not new_issue, key="screen_new_days"
        )

    screen_df = (
        df if cusip or not nist_only
    ...
    if cusip and results.empty:
        st.info(f"CUSIP {cusip} was not found in the currently combined ETF universe.")
    ...
    cols = [
        ...
        "Purchase Face ($)", "Est. Principal Cost ($)", "Annual Coupon Income ($)",
        "NonCallableProxy",
    ]
    cols = [c for c in cols if c in results.columns]
    st.dataframe(results[cols], use_container_width=True, height=650, hide_index=True)
...
def _candidate_selector(candidates, key):
    ...
    for _, row in candidates.iterrows():
        ...
        labels.append(
            ...
            f"YTW {float(row['Yield to Worst (%)']):.3f}% | gap {int(row['Maturity Gap Days'])}d"
        )
...
def _render_stacked_matches(muni, treasury, treasury_gap, prefix, extra_muni=None):
...
def render_nist_comparison(df):
    ...
    with r[0]:
        fed = st.number_input(
            "Client federal tax bracket (%)", 0.0, 60.0,
            value=None, step=0.1, key="nist_fed"
        )
    with r[1]:
        default_target = (pd.Timestamp.today().normalize() + pd.DateOffset(years=5)).date()
        target = st.date_input("Client target maturity", default_target, key="nist_target")
    with r[2]:
        states = st.multiselect(
    ...
    with r[3]:
        ig_only = st.checkbox("Investment grade only", key="nist_ig")
    ...
    candidates = nearest_muni_candidates(pool, target, states=states or None, limit=25)
    if candidates.empty:
        st.warning("No usable municipal bonds were found near that maturity.")
        return

    muni = _candidate_selector(candidates, "nist_choice")
    treasury = nearest_treasury(treasury_df, pd.Timestamp(muni["Maturity"]))
    if treasury is None:
        st.warning("No usable Treasury quote was available.")
        return
    treasury_gap = abs((pd.Timestamp(treasury["Maturity"]) - pd.Timestamp(muni["Maturity"])).days)
    _render_stacked_matches(muni, treasury, treasury_gap, "nist")

    if fed is None:
        st.info("Enter the client's **federal marginal tax bracket** to calculate the winner.")
        return
    ...
    comp = tax_equivalent_comparison(muni_ytw, treasury_yield, tax_rate)

    st.subheader("Tax Exempt Status for NIST")
    st.caption("NIST stands for No Income State Tax.")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Muni YTW / After-Tax", f"{comp['Muni After-Tax Yield (%)']:.3f}%")
    c2.metric("Taxable Yield Needed to Match Muni (TEY)", f"{comp['Muni Tax-Equivalent Yield (%)']:.3f}%")
    c3.metric("Treasury Gross Yield", f"{treasury_yield:.3f}%")
    c4.metric("Treasury After-Tax Yield", f"{comp['Treasury After-Tax Yield (%)']:.3f}%")

    spread = comp["After-Tax Spread (bps)"]
    if comp["Winner"] == "MUNICIPAL":
        winner = f"MUNICIPAL WINS // +{abs(spread):.1f} BPS AFTER FEDERAL TAX"
    elif comp["Winner"] == "TREASURY":
        winner = f"TREASURY WINS // +{abs(spread):.1f} BPS AFTER FEDERAL TAX"
    else:
        winner = "TIE // SAME AFTER-TAX YIELD"
    st.markdown(f'<div class="winner-box">{html.escape(winner)}</div>', unsafe_allow_html=True)
    ...
    with st.expander("Show 25 closest municipal candidates"):
        ...
        render_copy_cusips(candidates["CUSIP"].tolist(), "COPY 25 CANDIDATE CUSIPs")
...
def render_state_income_tax_comparison(df):
    ...
    with r1[0]:
        client_state = st.selectbox(
            "Client state of residence", ALL_US_STATES,
            index=default_indiana, key="state_client_state"
        )
    with r1[1]:
        state_rate_pct = st.number_input(
            "Client state income tax rate (%)", 0.0, 20.0,
            value=None, step=0.01, placeholder="e.g. 2.95",
            key="state_rate",
        )
    with r1[2]:
        federal_pct = st.number_input(
            "Client federal tax bracket (%)", 0.0, 60.0,
            value=None, step=0.1, placeholder="e.g. 22",
            key="state_fed",
        )
    with r1[3]:
        default_target = (pd.Timestamp.today().normalize() + pd.DateOffset(years=5)).date()
        target = st.date_input("Client target maturity", default_target, key="state_target")
    with r1[4]:
    ...
    with r2[0]:
        ig_only = st.checkbox("Investment grade only", key="state_ig")
    with r2[1]:
        home_state_exempt = st.checkbox(
            ...
            help=(
                "OFF treats the selected muni as state-taxable. ON applies 0% state tax "
                "only when the muni state equals the client's residence state. Verify the bond's tax treatment."
            ),
        )
    ...
    candidates = nearest_muni_candidates(
        pool, target, states=muni_states or None, limit=50
    )
    if candidates.empty:
        st.warning("No usable municipal bonds were found near that maturity for the selected state filter.")
        return
    ...
    _render_stacked_matches(
        ...
        extra_muni={
            ...
            "State Tax Treatment": "HOME-STATE EXEMPT" if state_exempt_applied else "STATE-TAXABLE",
        },
    )

    if federal_pct is None or state_rate_pct is None:
        st.info("Enter both the client's **federal tax bracket** and **state income-tax rate**.")
        return
    ...
    c2.metric("Muni After-State-Tax Yield", f"{muni_after_tax:.3f}%")
    c3.metric("Treasury Gross Yield", f"{treasury_gross:.3f}%")
    c4.metric("Treasury After-Federal-Tax Yield", f"{treasury_after_tax:.3f}%")

    if winner == "MUNICIPAL":
        winner_text = f"MUNICIPAL WINS // +{abs(spread_bps):.1f} BPS AFTER TAX"
    elif winner == "TREASURY":
        winner_text = f"TREASURY WINS // +{abs(spread_bps):.1f} BPS AFTER TAX"
    else:
        winner_text = "TIE // SAME AFTER-TAX YIELD"
    st.markdown(f'<div class="winner-box">{html.escape(winner_text)}</div>', unsafe_allow_html=True)
    ...
    st.caption(
        "Tax assumptions: municipal interest is treated as federally tax-exempt; Treasury interest is federally taxable and state/local income-tax exempt. "
        "Municipal state-tax treatment varies by state, issuer, residency, and bond type. The optional home-state exemption is a user-controlled assumption. "
        "AMT, local taxes, special reciprocity rules, and territory-specific treatment are not modeled unless included in the entered state rate."
    )
    ...
    with st.expander("Show 50 closest municipal candidates"):
        ...
        cols = [c for c in cols if c in candidates.columns]
        st.dataframe(candidates[cols], use_container_width=True, hide_index=True)
        render_copy_cusips(candidates["CUSIP"].tolist(), "COPY 50 CANDIDATE CUSIPs")
...
with top_right:
    if st.button("Refresh Muni Data", key="refresh_muni_data", use_container_width=True):
...
if used_session_cache:
    ...
    st.caption(
        f"DATA ENGINE // SESSION CACHE READY • {len(df):,} CUSIPs • loaded {age_minutes} min ago"
    )
