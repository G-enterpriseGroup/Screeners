...
EDIT THIS FILE FOR: app routing and the shared top-level tab/header shell.
DO NOT EDIT THIS FILE FOR: feature controls, calculations, OAuth or broker logic.
...
<style>
/* Only these keyed shell blocks own top-level spacing. Never target all
   stVerticalBlock descendants: feature widgets keep their own layout. */
.st-key-terminal_tab_shell {
    gap:4px !important;
    padding:0 !important;
    margin:0 !important;
}
.st-key-terminal_page_heading {
    gap:0 !important;
    padding:0 !important;
    margin:0 !important;
}
/* Browser-state hooks still mount and run, but reserve no visible row. */
.st-key-terminal_background_hooks { display:none !important; }
...
def _render_terminal_page_header(active_tab: str) -> None:
    ...
    _terminal_tab_layout emits the stylesheet on every render/session before
    this markup. Never gate shell styles behind a process-global import flag.
    """
...
@contextmanager
def _terminal_tab_layout(vault_key: str):
    """One compact navigation → header → content boundary for every route.

    ADDING A TAB:
    1. Add its stable key to DEFAULT_TAB_ORDER in src/tab_bar_v4.py.
    2. Add title/subtitle to _TERMINAL_PAGE_HEADERS above.
    3. Add the renderer to the dispatch inside this layout below. Do not emit
       another page heading or put hidden components between nav and heading.
    Feature sections stay inside terminal_page_content and keep their own CSS.
    Use st.html for style-only output, not spacer-producing markdown rows.
    """
    st.html(_TERMINAL_PAGE_HEADER_CSS)
    with st.container(key="terminal_tab_shell", gap=None):
        order, active = render_terminal_tab_bar(vault_key)
        with st.container(key="terminal_page_heading", gap=None):
            _render_terminal_page_header(active)
        with st.container(key="terminal_page_content"):
            yield order, active
