...
- configuring the optional licensed historical-price token used only for GEX IV/HV;
...
# ==============================
# GEX IV/HV HISTORICAL-PRICE SOURCE
# ==============================
def _configure_gex_volatility_source() -> bool:
    """Load an authorized Tiingo token server-side without exposing it to UI/state."""
    token = str(os.environ.get("TIINGO_API_TOKEN") or "").strip()
    if not token:
        try:
            token = str(
                st.secrets.get("TIINGO_API_TOKEN")
                or st.secrets.get("tiingo_api_token")
                or ""
            ).strip()
        except Exception:
            token = ""
    configure_tiingo_token(token)
    return bool(token)
...
def maybe_auto_refresh_on_login() -> bool:
    ...
    _configure_gex_volatility_source()
    (
        client,
        vault_key,
        touch,
