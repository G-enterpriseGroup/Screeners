...
EDIT THIS FILE FOR: E*TRADE connection strip, session controls, OAuth completion layout.
DO NOT EDIT THIS FILE FOR: broker token semantics, GEX, Risk Sizing, Holdings, or navigation.
...
def render_compact_etrade_connection(
    *,
    ...
    renew_notice = st.session_state.pop("_etrade_renew_notice", None)
    if renew_notice:
        st.toast(str(renew_notice))
    ...
    with renew_col:
        if st.button("RENEW", width="stretch", disabled=not connected, key="etrade_renew"):
            try:
                ...
                touch_session()
                # Keep renewal feedback out of the button column. A success/error
                # block inside this centered column changes the row height and
                # visibly shifts the RENEW button on click.
                st.session_state["_etrade_renew_notice"] = "E*TRADE session renewed."
                st.rerun()
            except etrade_error_type as exc:
                st.toast(f"E*TRADE renew failed: {exc}")
    with disconnect_col:
