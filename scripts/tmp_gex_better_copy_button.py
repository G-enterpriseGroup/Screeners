...
css = '''# GEX-only styling for Streamlit's real code-block clipboard control.
...
'''
if "_TRADINGVIEW_COPY_BUTTON_CSS" not in text:
    if marker not in text:
        raise SystemExit("TradingView section marker not found")
...
elif "COPY FOR TRADINGVIEW" not in text:
    raise SystemExit("Expected TradingView instruction/code block not found")
ui.write_text(text)

workspace = Path("src/gex_workspace_v2.py")
ws = workspace.read_text()
old_version = 'GEX_BUILD_VERSION = "v2026.09.17.07"'
new_version = 'GEX_BUILD_VERSION = "v2026.09.17.08"'
if old_version in ws:
    ws = ws.replace(old_version, new_version, 1)
elif new_version not in ws:
    raise SystemExit("Expected GEX build .07 or .08 not found")
...
if "## 2026-09-17 — Larger TradingView copy control" not in current:
    changelog.write_text(current + entry)

# Static contract checks before the workflow runs the full validators.
final_ui = ui.read_text()
assert "parser_text = text" in final_ui
assert "quoted_transport_ok = is_full or" in final_ui
assert "text.startswith('\"') and text.endswith('\"')" in final_ui
assert "_TRADINGVIEW_COPY_BUTTON_CSS" in final_ui
assert "COPY FOR TRADINGVIEW" in final_ui
assert "st-key-gexv3_bridge_code" in final_ui
assert "st.html(_TRADINGVIEW_COPY_BUTTON_CSS)" in final_ui
assert 'GEX_BUILD_VERSION = "v2026.09.17.08"' in workspace.read_text()
