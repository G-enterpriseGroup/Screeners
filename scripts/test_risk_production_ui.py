...
def main():
    ...
    assert app.toggle(key="risk_capital_tactical_switch").value is False
    ...
    assert liquid_app.toggle(key="risk_capital_tactical_switch").value is False
    ...
    assert tactical_app.toggle(key="risk_capital_tactical_switch").value is True
    ...
    switched = AppTest.from_string(FIXTURE, default_timeout=30).run()
    switched.toggle(key="risk_capital_tactical_switch").set_value(True).run()
    assert not switched.exception
    assert switched.session_state["risk_capital_source"] == "USE TACTICAL ROOM"
    assert switched.session_state["_risk_book_snapshot_v1"]["settings"]["risk_capital_source"] == "USE TACTICAL ROOM"
    switched.number_input(key="risk_stop_price").set_value(180).run()
    assert switched.toggle(key="risk_capital_tactical_switch").value is True
    switched.toggle(key="risk_capital_tactical_switch").set_value(False).run()
    assert not switched.exception
    assert switched.session_state["risk_capital_source"] == "USE LIQUID BALANCE ENTERED"
